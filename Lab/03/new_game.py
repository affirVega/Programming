import random
from tkinter import Tk, Button, Canvas, NW, W, TclError
...
def create_cells():
    ...
    if N > M:
        for shift in range(N-5):
            cm_diags.append(list())
            cm_rdiags.append(list())
            for i in range(M):
                cm_diags[shift].append(CellMatrix[i+shift][i])
                cm_rdiags[shift].append(CellMatrix[N-i-shift][i])
    else: # N <= M
        for shift in range(M-4):
            ...
            for i in range(N-shift):
                print(i+shift)
                cm_diags[shift].append(CellMatrix[i+shift][i])
                cm_rdiags[shift].append(CellMatrix[i+shift][i])
